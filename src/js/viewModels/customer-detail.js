define(['knockout','services/api','viewModels/base'], function(ko,api,BaseViewModel){
  ...
  function CustomerDetailViewModel(params){
    ...
    Promise.all([api.get('/customers/'+encodeURIComponent(id)),api.get('/accounts'),api.get('/transactions')]).then(function(result){
      ...
      return Promise.all(tx.map(function(t){return api.get('/transactions/'+encodeURIComponent(t.transactionId)+'/risk').catch(function(){return null;}).then(function(r){t.riskScore=r?Math.round(Number(r.riskScore||0)*100):0;t.riskCategory=r&&r.riskCategory||'LOW';t.explanation=r&&r.oneLineExplanation||'No risk explanation is available.';t.recommendation=r&&r.recommendation||'Continue routine monitoring.';t.displayDate=t.transactionDatetime?new Date(t.transactionDatetime).toLocaleString():'—';return t;});})).then(function(enriched){
        var score=Math.round((accounts.reduce(function(sum,a){return sum+Number(a.accountRiskScore||0);},0)/(accounts.length||1))*100);var highest=enriched.length?enriched.reduce(function(best,t){return t.riskScore>best.riskScore?t:best;}):null;
        self.accounts(accounts);self.transactions(enriched);self.customer({id:raw.id,name:'Customer '+raw.id.slice(0,8),type:raw.accountHolderType||'Unknown',kyc:raw.kycVerificationStatus||'Unknown',riskCountry:raw.riskCountryFlag?'Yes':'No',onboarded:raw.createdAt?new Date(raw.createdAt).toLocaleDateString():'—',score:Math.max(score,highest?highest.riskScore:0),risk:highest?highest.riskCategory:self.riskClass(score).toUpperCase(),accountCount:accounts.length,transactionCount:enriched.length,explanation:highest?highest.explanation:(accounts.length?'No flagged behavior was found across this customer\'s transactions.':'No accounts are linked to this customer yet.'),recommendation:highest?highest.recommendation:'Continue routine monitoring.'});
      });
